...
import { Upload, Trash2, FileText, Image, Monitor, BookOpen, CheckSquare, BarChart2, Loader2, Plus, X, ZoomIn } from 'lucide-react'
    ...
    >
      ...
      <img
        ...
        style={{
          ...
          cursor: 'default', boxShadow: '0 0 60px rgba(0,0,0,0.5)',
        }}
      />
    </div>
  )
      ...
      <div style={{
              ...
              {TIPOS.map(tipo => (
                <button
                  ...
                  style={{
                    ...
                    border: tipoSelecionado === tipo.value ? '1px solid #d4a843' : '1px solid rgba(255,255,255,0.08)',
                    background: tipoSelecionado === tipo.value ? 'rgba(212,168,67,0.1)' : 'rgba(255,255,255,0.02)',
            ...
            {tipoSelecionado && (
              <>
                <textarea
                  ...
                  placeholder={tipoSelecionado === 'relato' ? 'Escreva seu relato aqui...' : 'Adicione uma descrição (opcional)...'}
                  rows={3}
                  style={{
        ...
        {loading ? (
          ...
          <div style={{
            ...
            {readOnly ? 'Nenhuma evidência enviada pelo membro.' : 'Sem prova, não existiu. Adicione suas evidências acima.'}
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px' }}>
            {evidencias.map(ev => {
              const info = tipoInfo(ev.tipo)
              const isImage = ev.arquivo_url && /\.(jpg|jpeg|png|gif|webp|arw|cr2|nef)$/i.test(ev.arquivo_url)
              const isImageViewable = ev.arquivo_url && /\.(jpg|jpeg|png|gif|webp)$/i.test(ev.arquivo_url)
              ...
              return (
                <div key={ev.id} style={{
                  ...
                  {/* Preview de imagem — clica abre lightbox */}
                  {isImageViewable && (
                    <div
                      onClick={() => setLightboxUrl(ev.arquivo_url!)}
                      style={{
                    ...
                    >
                      <div style={{
                        ...
                        <ZoomIn size={24} color="white" style={{ opacity: 0.7 }} />
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ padding: '10px 12px' }}>
                    ...
                    {ev.arquivo_nome && !isImageViewable && (
                      <a
                        href={ev.arquivo_url!}
                        target="_blank"
                        rel="noreferrer"
