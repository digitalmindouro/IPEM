...
import { Plus, Users, ChevronRight, X, Loader2, CheckCircle, Clock, AlertCircle, Lock, Search } from 'lucide-react'
  ...
  return (
    <div style={{ display: 'flex', gap: '4px' }}>
      {[1, 2, 3, 4].map(num => {
        ...
        else if (status === 'em_andamento' || status === 'ajustar' || status === 'reaplicar') { bg = 'rgba(255,255,255,0.08)'; color = '#f5f0e8'; icon = CADERNO_LETRA[num-1] }
        else { icon = CADERNO_LETRA[num-1] }
...
export default function MentorDashboard({ profile, turmas: turmasIniciais, questionarios, todosProfiles }: Props) {
    ...
    try {
      const res = await fetch('/api/turmas/criar', {
        ...
        body: JSON.stringify({
          ...
          max_membros: maxMembros ? Number(maxMembros) : null,
          observacoes: obsturma || null,
        }),
      })
      const data = await res.json()
      if (res.ok) {
        toast.success('Turma criada!')
        const novaTurma = { ...data.turma, membros_turma: [] }
        ...
        toast.error(data.error || 'Erro ao criar turma.')
      }
  ...
  async function adicionarMembro(membroId: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const novoMembro: MembroTurma = {
          id: data.membro_turma.id,
          membro_id: membroId,
          data_entrada: new Date().toISOString(),
          concluiu: false,
          membro: membroProfile!,
        }
        const turmaAtualizada = {
          ...turmaSelecionada,
          membros_turma: [...turmaSelecionada.membros_turma, novoMembro],
        }
  ...
  async function removerMembro(membroId: string) {
    ...
    if (res.ok) {
      ...
      const turmaAtualizada = {
        ...turmaSelecionada,
        membros_turma: turmaSelecionada.membros_turma.filter(m => m.membro_id !== membroId),
      }
      setTurmaSelecionada(turmaAtualizada)
      setTurmas(prev => prev.map(t => t.id === turmaSelecionada.id ? turmaAtualizada : t))
    } else {
  ...
  const membrosFiltrados = todosProfiles.filter(p =>
    ...
    (p.nome.toLowerCase().includes(buscaMembro.toLowerCase()) ||
     p.email.toLowerCase().includes(buscaMembro.toLowerCase()))
  )
  ...
  const labelStyle = {
    ...
    color: '#7a7060', display: 'block', marginBottom: '6px',
  }
    ...
    <div className="max-w-4xl">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '32px' }}>
        <div>
          ...
          <p className="text-muted text-sm">{turmas.length} {turmas.length === 1 ? 'turma ativa' : 'turmas'}</p>
        </div>
        <button
          onClick={() => { setShowCriarTurma(true); setNomeTurma(nomeDefault) }}
          style={{
            display: 'flex', alignItems: 'center', gap: '6px',
            padding: '10px 18px', borderRadius: '4px', cursor: 'pointer',
            background: 'linear-gradient(135deg, #c8a96e 0%, #d4a843 100%)',
            border: 'none', color: '#1a1713', fontSize: '13px', fontWeight: '500',
          }}
        >
      ...
      {showCriarTurma && (
        <div style={{
          ...
          <div style={{
            background: '#1a1713', border: '1px solid rgba(212,168,67,0.2)',
            borderRadius: '8px', padding: '32px', width: '100%', maxWidth: '480px',
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <p style={{ fontFamily: 'Playfair Display, serif', fontSize: '20px', color: '#f5f0e8' }}>Nova Turma</p>
              <button onClick={() => setShowCriarTurma(false)} style={{ background: 'none', border: 'none', color: '#7a7060', cursor: 'pointer' }}>
                <X size={18} />
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                <label style={labelStyle}>Máximo de membros</label>
                <input type="number" value={maxMembros} onChange={e => setMaxMembros(e.target.value)} style={inputStyle} placeholder="Sem limite" />
              </div>
              <div>
                <label style={labelStyle}>Observações</label>
                <textarea value={obsturma} onChange={e => setObsTurma(e.target.value)} rows={2} style={{ ...inputStyle, resize: 'vertical' }} placeholder="Opcional..." />
              </div>
            </div>

            <div style={{ display: 'flex', gap: '8px', marginTop: '24px', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowCriarTurma(false)} style={{ padding: '10px 16px', borderRadius: '4px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#7a7060', cursor: 'pointer', fontSize: '13px' }}>
                Cancelar
              </button>
      ...
      {!turmaSelecionada ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {turmas.length === 0 ? (
            <div className="card-gold" style={{ textAlign: 'center', padding: '48px' }}>
              ...
              <p style={{ color: '#7a7060', fontSize: '13px', marginTop: '4px' }}>Clique em "Nova Turma" para começar.</p>
            </div>
          ) : turmas.map(turma => (
            <div
              key={turma.id}
        ...
        <div>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#7a7060', fontSize: '13px', background: 'none', border: 'none', cursor: 'pointer', marginBottom: '24px' }}
          >
            ← Voltar
          </button>
            ...
            <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ background: '#1a1713', border: '1px solid rgba(212,168,67,0.2)', borderRadius: '8px', padding: '32px', width: '100%', maxWidth: '440px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  ...
                  <button onClick={() => { setShowAdicionarMembro(false); setBuscaMembro('') }} style={{ background: 'none', border: 'none', color: '#7a7060', cursor: 'pointer' }}>
                    <X size={18} />
                  </button>
                </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  {membrosFiltrados.length === 0 ? (
                    <p style={{ color: '#7a7060', fontSize: '13px', textAlign: 'center', padding: '20px' }}>
                      {buscaMembro ? 'Nenhum resultado encontrado.' : 'Todos os membros já estão na turma.'}
                    </p>
                  ) : membrosFiltrados.map(p => (
                    <button
                      key={p.id}
                      onClick={() => adicionarMembro(p.id)}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {turmaSelecionada.membros_turma.length === 0 ? (
              ...
              <div key={mt.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <button
                  onClick={() => removerMembro(mt.membro_id)}
                  style={{ background: 'none', border: 'none', color: '#7a7060', cursor: 'pointer', padding: '4px', flexShrink: 0 }}
                >
                  <X size={14} />
                </button>
              </div>
